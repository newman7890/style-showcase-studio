-- 1. Upgrade resolve_delivery_fee to support partial/fuzzy town matching (e.g. "Ashongman estates" matches "Ashongman")
...
DECLARE
  v_fee NUMERIC;
BEGIN
  -- 1. Exact or partial town match within region
  IF _town IS NOT NULL AND length(trim(_town)) > 0 THEN
    ...
    WHERE is_active = true
      ...
      AND town IS NOT NULL AND length(trim(town)) > 0
      AND (
        lower(trim(town)) = lower(trim(_town))
        OR lower(trim(_town)) LIKE '%' || lower(trim(town)) || '%'
        OR lower(trim(town)) LIKE '%' || lower(trim(_town)) || '%'
      )
  ...
  -- 2. Exact city + region match (no town specified on fee row)
  ...
  -- 3. Region-only match (no city or town on fee row)
  SELECT fee INTO v_fee
  FROM public.delivery_fees
  WHERE is_active = true
    AND lower(trim(region)) = lower(trim(_region))
...
-- 2. Update recompute_order_total to strictly PRESERVE existing delivery_fee saved at checkout
...
BEGIN
  ...
  -- Prioritize the exact checkout delivery_fee if set on the order
